...
fn load_test_credentials() -> (String, String, String) {
    dotenv().ok();
    let client_id = env::var("AIRTEL_CLIENT_ID").expect("AIRTEL_CLIENT_ID must be set in .env file");
    let client_secret = env::var("AIRTEL_CLIENT_SECRET").expect("AIRTEL_CLIENT_SECRET must be set in .env file");
...
fn generate_unique_reference() -> String {
    format!("test_{}", Uuid::new_v4().to_string().replace("-", "")[..8].to_uppercase())
}

fn generate_unique_transaction_id() -> String {
    format!("tx_{}", Uuid::new_v4().to_string().replace("-", "")[..10].to_uppercase())
}
...
mod live_integration_tests {
    ...
    async fn test_collection_ussd_push() {
        ...
        match result {
            Ok(response) => {
                ...
                let status_result = collection.status(response.data.transaction.id.clone()).await;
    ...
    async fn test_disbursement() {
        ...
        match result {
            Ok(response) => {
                ...
                println!("Airtel Money ID: {}", response.data.transaction.airtel_money_id);
                println!("Status: {}", response.data.transaction.status);
    ...
    async fn test_remittance_eligibility() {
        ...
        let result = remittance.check_eligibility(
            TEST_PHONE_NUMBER.to_string(),
            TEST_AMOUNT,
            TEST_COUNTRY,
            TEST_CURRENCY
        ...
        match result {
            Ok(response) => {
                ...
                assert!(response.data.country.contains("KE") || response.data.country.contains("Kenya"));
                assert!(response.data.currency.contains("KES") || response.data.currency.contains("Kenyan"));
            }
            Err(e) => {
    ...
    async fn test_remittance_transfer_credit() {
        ...
        let result = remittance.money_transfer_credit(
            TEST_AMOUNT,
            ext_trid.clone(),
            TEST_PHONE_NUMBER.to_string(),
        ...
        match result {
            Ok(response) => {
                ...
                println!("Airtel Money ID: {}", response.data.transaction.airtel_money_id);
                println!("Status: {}", response.data.transaction.status);
                println!("External Transaction ID: {}", response.data.transaction.ext_trid);
    ...
    async fn test_collection_refund() {
        ...
        let ussd_result = collection.ussd_push(
            reference,
            TEST_PHONE_NUMBER.to_string(),
            TEST_AMOUNT,
            transaction_id
    ...
    async fn test_multiple_countries() {
        ...
        for (country, currency) in countries {
            println!("🌍 Testing with country: {:?}, currency: {:?}", country, currency);
            ...
            match result {
                Ok(balance) => {
                    ...
                    println!("Balance: {} {}", balance.data.balance, balance.data.currency);
                    assert!(balance.status.success);
                }
                Err(e) => {
    ...
    async fn test_concurrent_requests() {
        ...
        println!("✅ Concurrent requests test completed: {}/3 succeeded", success_count);
    }
}