...
mod mock_demo_tests {
    ...
    fn test_sdk_initialization() {
        ...
        for (country, expected_currency) in countries {
            ...
            println!(
                "   ✅ {}: {:?} -> {:?}",
                country, country, expected_currency
            );
        }
        ...
        let collection =
            airtel.collection(dummy_credentials.0.clone(), dummy_credentials.1.clone());
        let disbursement =
            airtel.disbursement(dummy_credentials.0.clone(), dummy_credentials.1.clone());
        let remittance =
            airtel.remittance(dummy_credentials.0.clone(), dummy_credentials.1.clone());
