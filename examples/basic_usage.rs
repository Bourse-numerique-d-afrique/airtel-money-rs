...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match disbursement.disburse(
        ...
        Ok(response) => {
            println!("✅ Disbursement successful: {}", response.data.transaction.id);
        },
    ...
    match remittance.check_eligibility(
        "254700000000".to_string(),
        100,
        Country::Kenya,
        Currency::KES
        ...
        Ok(response) => {
            println!("✅ Eligibility check: {}", if response.data.eligible { "Eligible" } else { "Not eligible" });
        },
