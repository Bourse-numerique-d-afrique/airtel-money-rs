use crate::{authorization::get_valid_access_token, requests::cash_in_payment_request::{CashInPaymentRequest, Subscriber, Transaction, AdditionalInfo}, responses::cash_in_response::CashInResponse, Country, Currency, Environment};
...
impl CashIn {
     ...
     */
    pub async fn cash_in(&self, msisdn: String, amount: i32, transaction_id: String, reference: String, pin: String, remark: String) -> Result<CashInResponse, Box<dyn std::error::Error>> {
    ...
    /*
        * Get the status of a cash in
        ...
        @return Result<CashInResponse, Box<dyn std::error::Error>>
     */
    pub async fn get_status(&self, id: String) -> Result<CashInResponse, Box<dyn std::error::Error>> {
        let client = reqwest::Client::new();
        let access_token = get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
