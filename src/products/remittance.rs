...
impl Remittance {
    ...
    */
    pub async fn check_eligibility(
        ...
        country: Country,
        currency: Currency,
    ) -> Result<RemittanceEligibilityResponse, Box<dyn std::error::Error>> {
        let token =
        ...
        let res = client
            ...
            .body(RemittanceEligibilityRequest {
                amount,
                country,
                currency,
                msisdn,
    ...
    */
    pub async fn money_transfer_status(
        &self,
        ext_tr_id: String,
    ) -> Result<RemittanceStatusResponse, Box<dyn std::error::Error>> {
        let token =
            get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
        let client = reqwest::Client::new();
        let res = client
            ...
            .header("Content-type", "application/json")
            .header("Accept", "*/*")
            .body(RemittanceTransferStatusRequest::new(
                self.country,
                ext_tr_id,
            ))
            .send()
    ...
    */
    pub async fn money_transfer_credit(
        &self,
        amount: i32,
        ext_trid: String,
        msisdn: String,
        ...
        let res = client
            ...
            .header("Content-type", "application/json")
            .header("Accept", "*/*")
            .body(RemittanceTransferCreditRequest {
                amount,
    ...
    */
    pub async fn refund(
        ...
        txn_id: String,
        pin: String,
    ) -> Result<RemittanceRefundResponse, Box<dyn std::error::Error>> {
        let token =
            get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
        let client = reqwest::Client::new();
        let res = client
