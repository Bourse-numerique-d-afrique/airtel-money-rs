...
use crate::{ authorization::get_valid_access_token, requests::disbursement_payment_request::{DisbursementPaymentRequest, Payee, Transaction}, responses::disbursement_payment_response::DisbursementPaymentResponse, Country, Currency, Environment};
...
impl Disbursement {
     ...
     */
    pub async fn disburse(&self, msisdn: String, amount: i32, transaction_id: String, reference: String, pin: String) -> Result<DisbursementPaymentResponse, Box<dyn std::error::Error>> {
        ...
        .body(
            DisbursementPaymentRequest {
                ...
                reference,
                pin,
                transaction: Transaction { amount, id: transaction_id },
            }
        );
    ...
    /*
        * Get Status
        ...
        @return Result<DisbursementPaymentResponse, Box<dyn std::error::Error>>
     */
    pub async fn get_status(&self, id: String) -> Result<DisbursementPaymentResponse, Box<dyn std::error::Error>> {
        let client = reqwest::Client::new();
        let access_token = get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
