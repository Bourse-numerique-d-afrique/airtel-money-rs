...
impl Collection {
    ...
    pub async fn ussd_push(&self, reference: String, msisdn: String, amount: i32, id: String) -> Result<CollectionUSSDResponse, Box<dyn std::error::Error>> {
    ...
    pub async fn refund(&self, airtel_money_id: String) -> Result<CollectionRefundResponse, Box<dyn std::error::Error>> {
        let client = reqwest::Client::new();
        let access_token = get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
    ...
    pub async fn status(&self, transaction_id: String) -> Result<CollectionStatusResponse, Box<dyn std::error::Error>> {
        let client = reqwest::Client::new();
        let access_token = get_valid_access_token(self.environment, &self.client_id, &self.client_secret).await?;
        let res = client.get(format!("{}/standard/v1/payments/status/{}", self.environment, transaction_id))
        .bearer_auth(access_token.access_token)
        .header("Content-Type", "application/json")
        .header("X-Country", self.country.to_string())
        .header("X-Currency", self.currency.to_string())
